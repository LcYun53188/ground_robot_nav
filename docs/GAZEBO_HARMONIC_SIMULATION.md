...
坡仍应按实车允许坡度单独验证，不应仅因 nvblox 允许最高 `30deg` 地面坡度，就视为
底盘具备相同的物理爬坡能力。
...
坡面深度不会直接全部送入 nvblox。`traversable_depth_filter` 先保留墙面、台阶边界和
障碍物侧面，把连续且不超过 `30deg` 的地面、坡面及坡顶编码为无效深度 `0`。过滤
窗口覆盖车体坐标高度 `[-1.50, 0.75]m`，避免车辆在坡顶仍有俯仰角时，把远处同一
支撑面误判成低处障碍。连续性按相邻像素在 `base_link` 中的真实水平距离和 `30deg`
坡度计算，并额外允许 `0.015m` 深度栅格残差；不使用固定光学深度差门限，因此侧视
坡面也不会被误判。`5cm` 台阶不满足这个局部坡度模型，边界仍会保留。

nvblox 对这些深度为 `0` 的像素启用 `0.8` 无效深度 TSDF 衰减。这样早期视角偶然
写入的坡面条纹会在后续确认该处为可通行面时被清除，而持续观测到的真实墙面和障碍
边界仍会正常融合。该参数在 nvblox 创建 mapper 时生效；修改配置后需要重启导航，
仅清空 Nav2 costmap 不能删除已经保存在 nvblox TSDF 中的旧条纹。
...
`(0.18, 0, 0.16)`，其 Gazebo 视轴 `+X` 与车体前进方向一致，并绕车体 `Y` 轴向
地面俯视 `18deg`。该外参与 Gazebo 模型保持一致。ROS 图像使用独立的标准光学坐标系
