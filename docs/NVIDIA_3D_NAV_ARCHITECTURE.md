...
- `multi_mapper.max_ground_plane_slope_deg: 30.0`
...
内采用带无效样本上限的多数证据判定。朝上且不大于 30° 的局部
坡面从二维障碍中剔除；从坡底观察导致 TSDF 梯度翻转时，仅 5-30° 的明显坡度允许按
缓坡处理，近水平的朝下表面继续作为悬空障碍。大于 30°、缺少可靠缓坡证据，或陡坡
...
至少两个邻格构成 5-30° 连续坡面、2 cm 高度残差内无突变且没有可靠障碍法向时解除
